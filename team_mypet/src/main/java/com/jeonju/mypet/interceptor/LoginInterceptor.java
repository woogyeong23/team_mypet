...
public class LoginInterceptor implements HandlerInterceptor {
	...
	/* 잠시 주석처리
	 * @Override//사용자 요청이 Controller로 전달되기 전에 요청을 가로채어서 메소드 실행 public boolean
	 * preHandle(HttpServletRequest request, HttpServletResponse response, Object
	 * handler) throws Exception { HttpSession session = request.getSession();
	 * String member_id = (String)session.getAttribute("member_id");
	 * 
	 * if(member_id == null) {
	 * response.sendRedirect(request.getContextPath()+"/login.do"); return
	 * false;//Controller로 사용자 요청이 전달되지 못하도록 함 }
	 * 
